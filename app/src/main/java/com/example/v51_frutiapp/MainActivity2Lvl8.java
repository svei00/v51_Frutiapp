...
public class MainActivity2Lvl8 extends AppCompatActivity {
    ...
    int iScore, iRndm1, iRndm2, iRes, iChances = 3, iResiduo; // Las oportunidades se inicializan en tres, Se le agregó también la variable iResudio para hacer divisiones que den solo números enteros
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_main_activity2_lvl8);

        // Indicamos al Usuario en que nivel esta
        Toast.makeText(this, "Nivel 8 - Sumas, Restas, Multiplicaciones y Divisiones", Toast.LENGTH_SHORT).show();
    ...
    public void numRandom (){
        ...
        if (iScore <= 100) { // Cambiar el 60 por el final o continuar con divisiones
            iRndm1 = (int) (Math.random() * 10);
            iRndm2 = (int) (Math.random() * 10);
            iResiduo = iRndm1 % iRndm2;

            // Estructura Condicional para Alternar Operaciones
            if (iRndm1 >= 0 && iRndm1 <= 2) {
            ...
            } else if (iRndm1 >= 3 && iRndm1 <= 5) {
                // Agregamos la Operación de Resta
                iRes = iRndm1 - iRndm2;

                // Fijamos la Operación
                ivOperador.setImageResource(R.drawable.resta);
            } else if (iRndm1 >= 6 && iRndm1 <=7) {
                // Operación para la Multiplicación
                iRes = iRndm1 * iRndm2;

                // Definimos el Operador
                ivOperador.setImageResource(R.drawable.multiplicacion);
            } else {
                // Agregamos otro IF para que el primer número no sea cero y de solo divisiones enteras
                if (iRndm1 != 0 && iResiduo == 0) {
                    iRes = iRndm1 / iRndm2;

                    // Definimos el Operador
                    ivOperador.setImageResource(R.drawable.division);
                }
            }
            ...
            Intent lvl2 = new Intent(this, MainActivity.class);

            /* // Estas Líneas de Código estan Comentadas por si se elabora los niveles
            7 y 8 (División y Todas las Operaciones), donde en el hipotético Nivel 8 se eliminarian las líneas
            de código definitivamente.
            ...
            // COmo idea se puede crear un Activity con el Resultado Final así
            como un botón para iniciar una nueva partida
             */

            // En caso de Finalizar el Juego se puede poner el Toast o un Nuevo Activity con el fin del juego
            Toast.makeText(this, "¡Felicidades, " + sPlayer + " eres un Genio!", Toast.LENGTH_SHORT).show();
            Toast.makeText(this,"¡Nadie como tú! ", Toast.LENGTH_LONG).show();
